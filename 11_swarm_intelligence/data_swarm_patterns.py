...
class DataSwarmEngine:
    ...
    def verify_message_signature(self, message: SwarmMessage) -> bool:
        ...
        expected_signature = hmac.new(
            sender_key, content_str.encode("utf-8"), hashlib.sha256
        ).hexdigest()
    ...
    def ensure_packet_redundancy(self, packet: DataPacket) -> bool:
        with self.lock:
            ...
            available_peers = [
                pid
                for pid in self.peer_nodes.keys()
            ...
            for peer_id in available_peers[:needed]:
                ...
                self.send_p2p_message(
                    peer_id, MessageType.DATA_PACKET, replication_payload
                )
    ...
    def assign_node_role(
        ...
        if not self.verify_node_capacity(node_id, capacity):
            self.record_failure(
                ...
                f"Node {node_id} capacity insufficient",
            )
            return False

        if self.check_role_conflict(node_id, role):
            self.record_failure(
                FailureType.ROLE_CONFLICT,
                f"Role conflict for {node_id}: {role}",
            )
            return False
...
if __name__ == "__main__":
    ...
    def test_basic_swarm_functionality():
        ...
        node2.add_peer_node(
            "node_001", "test-secret-key-123", SwarmNodeRole.COORDINATOR
        )
    ...
    def test_p2p_messaging(node1, node2, node3):
        ...
        if node1.send_p2p_message(
            "broadcast", MessageType.HEARTBEAT, broadcast_payload
        ):
    ...
    def test_packet_replication(node1, node2, node3):
        ...
        if packet_id:
            ...
            messages_in_network = len(
                [m for m in node2.message_queue if "packet_data" in m.payload]
    ...
    def run_comprehensive_test():
        ...
        try:
            ...
            print("✓ Swarm nodes initialized: 3/3")
            ...
            overall_success = (
                len(valid_packets) >= 3
                and message_success >= 3
                and replication_success
